...
var Openfire = {};
        ...
        this.host = url.indexOf("/") < 0 ? url : url.split("/")[2];
                this.protocol = url.indexOf("/") < 0 ? "ws:" : (url.split("/")[0] == "http:") ? "ws:" : "ws:";
//this.protocol = "http:";
                this.jid = "";
                this.resource = "tracker";
                var dt = new Date();
//    this.resource = "Unichat_"+dt.getMinutes()+"_"+dt.getHours()+"_"+dt.getSeconds()+"_"+Math.random();
                this.streamId = null;
                // handler lists
...
this.streamId = null;
        ...
        this.jid = jid.indexOf("/") > - 1 ? jid : jid + '/' + this.resource;
                ...
                this.url = this.protocol + "//" + this.host + "/ws/server?username=" + this.username + "&password=" + this.pass +
                "&resource=" + this.resource;
//        this.url = "wss://" + this.host + "/?username=" + this.username + "&password=" + this.pass + "&resource=" + this.resource;
//        console.log("Connecting URL:"+this.url);
        ...
        window.openfireWebSocket.sendRaw(" ")
        }, 10000);
        },
        /** Function: attach
        ...
        var elem;
                ...
                elem = this._textToXML(packet.data);
                        var xml = packet.data;
                        var res = xml.match(/body/g);
                        if (res !== null){
                $.ajax({
                url:"getUserDetailsXmpp.php",
                        data:{'values':packet.data},
                        type:'post',
